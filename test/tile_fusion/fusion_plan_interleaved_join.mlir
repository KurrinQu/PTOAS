// RUN: { ptoas %s --enable-op-fusion --pto-backend=a5vm --pto-arch=a5 --op-lib-dir=%S/../../oplib/level3 --print-ir-after-all --print-ir-after-all-func-filter=fusion_plan_interleaved_join -o /dev/null 2>&1 || true; } | FileCheck %s
...
// are not physically contiguous in IR. That makes the need for a later
// scheduling pass visible without forcing FusionPlanPass to reorder ops.

module {
  func.func @fusion_plan_interleaved_join(
...
// CHECK-LABEL: func.func @fusion_plan_interleaved_join(
// CHECK: pto.trowexpandmul{{.*}}pto.fusion.group_id = 0 : i64{{.*}}pto.fusion.order = 0 : i64
// CHECK: pto.tadd ins(%arg2, %arg3
// CHECK-NOT: pto.fusion.group_id
// CHECK: pto.trowexpandmul{{.*}}pto.fusion.group_id = 0 : i64{{.*}}pto.fusion.order = 1 : i64
