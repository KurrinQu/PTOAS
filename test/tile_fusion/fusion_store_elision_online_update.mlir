...
// CHECK: %[[STRAIGHT_SRC:[0-9]+]] = pto.vlds %4[%c0]
// CHECK: %[[STRAIGHT_SRC0:[0-9]+]] = pto.vlds %0[%c0]
// CHECK: %[[STRAIGHT_MAX:[0-9]+]] = pto.vmax %[[STRAIGHT_SRC]], %[[STRAIGHT_SRC0]], %mask
// CHECK: pto.vsts %[[STRAIGHT_MAX]], %9[%c0], %mask
// CHECK-NOT: pto.vlds %9[%c0]
// CHECK: %[[STRAIGHT_SUB:[0-9]+]] = pto.vsub %{{[0-9]+}}, %[[STRAIGHT_MAX]], %mask
// CHECK: %[[STRAIGHT_EXP0:[0-9]+]] = pto.vexp %[[STRAIGHT_SUB]], %mask
// CHECK: pto.vsts %[[STRAIGHT_EXP0]], %12[%c0], %mask
// CHECK-NOT: pto.vlds %12[%c0]
// CHECK: %[[STRAIGHT_ADD:[0-9]+]] = pto.vadd %{{[0-9]+}}, %{{[0-9]+}}, %mask
// CHECK: pto.vsts %[[STRAIGHT_ADD]], %17[%c0], %mask
...
// CHECK: pto.vsts %[[G1_MUL0]], %8[%{{[^]]+}}], %mask
// CHECK: %[[G1_LOAD1:[0-9]+]] = pto.vlds %2[%{{[^]]+}}]
// CHECK: %[[G1_MUL1:[0-9]+]] = pto.vmul %[[G1_LOAD1]], %{{[0-9]+}}, %mask
// CHECK: pto.vsts %[[G1_MUL1]], %6[%{{[^]]+}}], %mask
// CHECK: %[[G1_RELOAD0:[0-9]+]] = pto.vlds %8[%{{[^]]+}}]
// CHECK: %[[G1_RELOAD1:[0-9]+]] = pto.vlds %6[%{{[^]]+}}]
// CHECK: %[[G1_ADD:[0-9]+]] = pto.vadd %[[G1_RELOAD0]], %[[G1_RELOAD1]], %mask
