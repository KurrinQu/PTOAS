// RUN: { ptoas %s --enable-op-fusion --pto-arch=a5 --op-lib-dir=%S/../../oplib/level3 --print-ir-after-all --print-ir-after-all-func-filter=op_scheduling_negative_ssa -o /dev/null 2>&1 || true; } | FileCheck %s --check-prefix=SSA
// RUN: { ptoas %s --enable-op-fusion --pto-arch=a5 --op-lib-dir=%S/../../oplib/level3 --print-ir-after-all --print-ir-after-all-func-filter=op_scheduling_negative_call_boundary -o /dev/null 2>&1 || true; } | FileCheck %s --check-prefix=CALL
// RUN: { ptoas %s --enable-op-fusion --pto-arch=a5 --op-lib-dir=%S/../../oplib/level3 --print-ir-after-all --print-ir-after-all-func-filter=op_scheduling_negative_region -o /dev/null 2>&1 || true; } | FileCheck %s --check-prefix=REGION
