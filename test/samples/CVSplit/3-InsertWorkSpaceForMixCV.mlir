module {
  ...
  func.func @cube_matmul_vadd_2d(
    ...
    // 3) DMA load (GM -> UB) : DPS needs outs(%init)
    ...
    // 4) Matmul : DPS op is pto.matmul_dps with outs(%init)
    ...
    // 7) Workspace
    // If you keep it, it will require lowering/bufferization later.)
    %workspace = memref.alloc() : memref<64x64xf32>

    // 8) Store back to GM tile : DPS store needs outs(%init)
    ...
    // 9) Insert the result back into the main output tensor
