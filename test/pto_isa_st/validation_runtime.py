...
def _infer_scalar_input_from_kernel(
    meta: CaseMeta, kernel_cpp: str | Path | None = None
) -> str | None:
...
def scalar_input_names(meta: CaseMeta) -> tuple[str, str]:
    if len(meta.inputs) != 2:
        raise ValueError(
            f"expected exactly two inputs for scalar case, got {meta.inputs}"
        )
    scalar_candidates = [
        name for name in meta.inputs if meta.elem_counts.get(name) == 1
    ]
