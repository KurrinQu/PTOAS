// RUN: ptoas --enable-op-fusion --pto-arch=a5 --pto-backend=vpto --print-ir-after-all --print-ir-after-all-func-filter=cleanup_preserves_loops %s -o /dev/null > %t 2>&1
// RUN: awk '/IR Dump After PTOVPTOIfCanonicalize/{found=1} found{if (found > 1 && /IR Dump After /) exit; print; found=2}' %t | FileCheck %s
