// RUN: ptoas --pto-backend=a5vm --print-ir-after-all --print-ir-after-all-func-filter=direct_tadd %s -o /dev/null > %t 2>&1
...
// CHECK-NOT: scf.if
// CHECK-NOT: pto.tadd
// CHECK-NOT: pto.a5vm_lowering_choice

module {
  func.func @direct_tadd(%src0_raw: memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>, %src1_raw: memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>, %dst_raw: memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>) attributes {pto.a5vm_version_selection_applied} {
    ...
    pto.tadd ins(%src0, %src1 : memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>, memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>) outs(%dst : memref<1x16xf32, strided<[16, 1], offset: ?>, #pto.address_space<vec>>) {pto.a5vm_lowering_choice = #pto.a5vm_lowering_choice<update_mode = no_post_update, loop_shape = two_d>}
    return
  }
}
