# RMSNorm FP32 5120 Persistent Fragment Case

The PTODSL source is in `kernel.py`.

This PTODSL source is the persistent-fragment variant of the TileLang
RMSNorm kernel for `batch=4096` and `d=5120`.

- The weight buffer is loaded from GM to UB once.
- A SIMT init section copies the 20 valid `f32` weight elements into the
  persistent fragment.
- Each token SIMT section resumes and re-keeps those 20 elements.
- The remaining 12 elements of the 32-element physical fragment are layout
