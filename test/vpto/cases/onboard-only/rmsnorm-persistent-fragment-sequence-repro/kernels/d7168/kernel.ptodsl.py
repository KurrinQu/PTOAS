...
@pto.jit(name="rmsnorm_d7168_kernel", kernel_kind="vector", target="a5", mode="explicit")
def rmsnorm_d7168_kernel(RSTD: pto.ptr(pto.f32, "gm"), W: pto.ptr(pto.f32, "gm"), X: pto.ptr(pto.f32, "gm"), Y: pto.ptr(pto.f32, "gm"), eps: pto.f32):
  ...
  pto.mte_gm_ub(W, pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), 0, 28672, nburst=(1, 28672, 28672))
  ...
  with pto.for_(0, 64, step=1) as t:
    ...
    pto.mte_gm_ub(pto.addptr(X, (t * 458752) + (pto.get_block_idx() * 7168)), pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), ((t & 1) * 8192) + 7168), 0, 28672, nburst=(1, 28672, 28672))
    ...
    with pto.simt(256, 1, 1):
      ...
      for i_1 in pto.static_range(0, 16):
        scalar.store(scalar.load(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), ((((t & 1) * 8192) + (i_1 * 512)) + (simtvf_tx_1 * 2)) + 7168, contiguous=2), x_frag, i_1 * 2)
      ...
      scalar.store(pto.simt_allreduce_sum(scalar.load(sum_sq, 0), threads=256, scale=1, thread_offset=0, scratch=pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), 39936)), sum_sq, 0)
      ...
      for i_3 in pto.static_range(0, 16):
        if i_3 < 14:
          scalar.store((scalar.load(x_frag, i_3 * 2, contiguous=2) * pto.Vec(pto.f32, 2, init=(float.fromhex('0x1p+0') / pto.sqrt(var)))) * scalar.load(w_frag_1, i_3 * 2, contiguous=2), pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), ((((t & 1) * 8192) + (i_3 * 512)) + (simtvf_tx_1 * 2)) + 23552)
    pto.set_flag("V", "MTE3", event_id=t & 1)
    pto.set_flag("V", "MTE2", event_id=t & 1)
    pto.wait_flag("V", "MTE3", event_id=t & 1)
    pto.mte_ub_gm(pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), ((t & 1) * 8192) + 23552), pto.addptr(Y, (t * 458752) + (pto.get_block_idx() * 7168)), 28672, nburst=(1, 28672, 28672))
