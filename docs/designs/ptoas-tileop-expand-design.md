...
### 1.2 PTOAS 中Tile Lib实现的挑战

PTOAS 中目前设计两层粒度的中间表示：

- **PTO TileOp**：面向上层用户的高层抽象，操作对象是 `tile_buf`，一条指令表达完整的 tile 语义（如 `pto.tadd`、`pto.tmul`、`pto.tload`）。
- **vPTO**：面向底层硬件的指令接口，操作对象是 `vreg`/`ptr`，需要显式循环、显式寄存器宽度、显式 mask 处理（如 `pto.vadd`、`pto.vlds`、`pto.vsts`）。

Tile Lib 的一种实现方式是直接使用 vPTO 编写。以 `pto.tadd`（逐元素加法）为例，在 TileOp层只需一条指令：
...
而用 vPTO 实现同样的语义（`dtype=f32, rows=16, cols=64`），需要展开为完整的vpto指令循环：
...
直接基于 vPTO 开发 Tile Lib 面临以下困难：

1. **PTOAS语法门槛高**：需要熟悉 `memref`、`index`、`strided` 等数据类型和语法，使用PTOAS的方式定义变量、表达运算和控制流。
2. **参数组合无法穷举**：`dtype` 有 f16/f32/bf16 等，`rows`/`cols` 可以是任意正整数，为每种 `(op, dtype, rows, cols, layout)` 组合手写实现不可行。

因此，直接基于 PTO vPTO 开发 Tile Lib，技术难度大且工作无法收敛。
...
Tile Lib 的实现。库开发者使用 Python 编写 vkernel 函数，PTOAS 编译器在编译时
...
1. **用 TileLang Python DSL 编写 vkernel**：以 `@pto.vkernel` 装饰器声明匹配元数据
   ...
   `pto.Tile` 数据类型和基础vpto指令（`make_mask` / `vlds` / `vsts` / `vadd` / …）
   按 Tile 指令语义开发Tile Lib实现。
2. **编译器匹配并特化 vkernel**：PTOAS 遇到 Tile op 时，通过 DSL 提供的
   ...
   以 `tile_buf` 为形参的Tile Lib实现。
3. **inline 到调用点**：特化后的Tile Lib 以 `func.call` 形式插入到原 Tile op 的位置，
...
`pto.Tile` 表示一个带有布局和配置信息的数据块，对应PTOAS中的 `!pto.tile_buf` 类型。
...
`valid_shape`（可能是动态值）时，生成 `scf.for` 循环。

**VF作用域**：本方案的 vkernel 统一使用 `advanced=True`，由编译器的 Scope Inference Pass
...
PTOAS 编译器的输入可以是 Tile 指令、vpto指令、或两者的混合。完整的编译 pipeline 如下：

```
输入：TileOp / vpto指令 / TileOp + vpto指令混合
       ↓
  ...
  Fold TileBuf Intrinsics   ← 折叠 tile_buf / tensor_view intrinsic，解析到具体值
       ↓
  VF Fusion                 ← 合并相邻Tile Lib，消除中间 UB 读写
       ↓
  LLVM IR
```

Tile 指令到vpto指令的展开由三个 pass 协作完成：

1. **Expand TileOp**：核心 pass。调用 TileLang Python DSL 实例化模板库，生成以 `tile_buf` 为参数的Tile Lib实现，将原 Tile op 替换为对该函数的 `func.call`。
...
─────────────────────────────────────
  ...
  Python DSL 查找匹配的 @vkernel 模板，按 Tile / View / Scalar 参数约定填入具体参数进行特化
  输出实例化后的函数（以 tile_buf / partition_tensor_view / scalar 为参数）
  解析实例化后的文本，克隆到目标 Module，写入缓存
...
TileLang DSL 实例化后，生成的函数结构如下（以 `pto.tadd`、`dtype=f32`、`shape=(16,64)` 为例）：
...
func.func @__pto_tilelang_tadd_f32_16_64(
  ...
  // 4. vpto指令循环体：按行遍历，按 vreg 宽度分块，带尾部 mask
  pto.vecscope {
...
以下展示一个完整的 `pto.tadd` 从 TileOp 到vpto指令的变换过程。
...
折叠后得到最终的纯vpto 指令，不再包含任何 tile_buf 引用：
...
这与 1.2 节中手写的 vPTO 实现等价，但由 Python DSL 模板自动生成。
...
| 模板实例化 | 调用 Python DSL，传入具体 `tile_buf` 类型，获取实例化后的函数 |
| 解析与 inline | 解析生成的文本，inline 到调用点，绑定参数 |
...
- Python DSL 模板编写和实例化的单元测试
  ...
  // pto.tadd should be lowered to vector-style VPTO instr.
...
- Expand TileOp pass 的端到端测试（`pto.tadd` → Vector IR）
  ...
  ./build/tools/ptoas/ptoas test/basic/expand_tile_op_tilelang.pto \
    ...
    -o - \
    > add.ll \
    2> /tmp/expand_tile_op_tilelang.log
  ```

  说明：
  - `stderr` 中的 `/tmp/expand_tile_op_tilelang.log` 保存 `--print-ir-after-all` 打印的各阶段 VPTO IR，可用于检查模板是否已经从 `pto.tadd` 展开为vpto指令。
  ...
  若上述命令成功生成 `add.o`，则说明当前 `pto.tadd` 的Tile Lib已经完成：
  - TileLang 模板实例化；
  - `pto.tadd -> vPTO -> LLVM IR` 的端到端 lowering；
  - Bisheng 设备侧编译校验。
