...
static bool requiresExplicitVPTOLoweringChoice(Operation *op) {
  auto func = op->getParentOfType<func::FuncOp>();
  return func && func->hasAttr(kPTOVersionSelectionAppliedAttrName);
}
...
FailureOr<VPTOLoweringStrategy>
resolveVPTOLoweringStrategyForOp(Operation *op,
                                 VPTOLoweringStrategy fallbackStrategy) {
  if (!op->hasAttr(kPTOLoweringChoiceAttrName)) {
    if (requiresExplicitVPTOLoweringChoice(op)) {
      op->emitOpError()
          << "requires '" << kPTOLoweringChoiceAttrName
          << "' before PTOToVPTO in VPTO fusion mainline, but the attribute is "
             "missing";
      return failure();
    }
  ...
  return convertPTOUpdateModeToVPTOLoweringStrategy(choice->getUpdateMode());
}
...
struct PTOToVPTOPass : public impl::PTOToVPTOBase<PTOToVPTOPass> {
  ...
  void runOnOperation() override {
    ...
    for (Operation *op : tensorPipelineOps) {
      if (!op->getBlock())
        continue;
      FailureOr<PTOLoweringChoiceAttr> effectiveChoice = failure();
      if (requiresExplicitVPTOLoweringChoice(op) &&
          op->hasAttr(kPTOLoweringChoiceAttrName)) {
        effectiveChoice = getVPTOLoweringChoiceAttr(op);
        if (failed(effectiveChoice)) {
          sawFailure = true;
          continue;
        }
      }
      FailureOr<VPTOLoweringStrategy> effectiveStrategy =
          resolveVPTOLoweringStrategyForOp(op, *loweringStrategy);
      ...
      if (failed(lowerTensorPipelineOp(op, rewriter, *effectiveStrategy))) {
        if (succeeded(effectiveChoice)) {
          op->emitOpError()
              << "failed to lower with '" << kPTOLoweringChoiceAttrName
              << "' = " << *effectiveChoice;
        }
        sawFailure = true;
      }
