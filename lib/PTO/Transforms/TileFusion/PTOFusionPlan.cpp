...
static bool isProvenIterationDomain(const pto::FusionBlockAnalysis &blockAnalysis,
...
static unsigned countEdgesFromGroup(const pto::FusionBlockAnalysis &blockAnalysis,
...
public:
  ...
  evaluateAppend(const PlanningContext &ctx,
    ...
    // Local boundaries such as `pto.treshape` are already excluded from
    // computeNodes by pre-fusion analysis. Their presence still creates a
    // block-order gap, which conservatively breaks only the chain that passes
    // through them without globally blocking later unrelated groups.
