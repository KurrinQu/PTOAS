...
static bool hasDependencyOnLocalBoundary(Operation *movingOp,
  ...
  return sharesAnyValue(boundarySemantics.tileOutputs, movingSemantics.tileInputs) ||
         sharesAnyValue(boundarySemantics.tileOutputs, movingSemantics.tileOutputs) ||
         sharesAnyValue(movingSemantics.tileOutputs, boundarySemantics.tileInputs) ||
         sharesAnyValue(movingSemantics.tileOutputs, boundarySemantics.tileOutputs);
}
...
static LogicalResult collectScheduledGroups(Block &block,
                                            SmallVectorImpl<ScheduledGroup> &groups) {
  DenseMap<int64_t, unsigned> groupIndexById;

  unsigned originalIndex = 0;
  for (Operation &op : block) {
    if (hasIncompleteFusionMetadata(&op)) {
      op.emitError("expected pto.fusion.group_id and pto.fusion.order to either both exist or both be absent");
      return failure();
    }
  ...
  for (ScheduledGroup &group : groups) {
    ...
    for (const GroupMember &member : group.members) {
      if (classifySchedulingBarrier(member.op) != SchedulingBarrierKind::Movable) {
        member.op->emitError("fusion scheduling metadata must only annotate movable compute ops");
        return failure();
      }
      if (previousOrder && *previousOrder == member.order) {
        member.op->emitError("duplicate pto.fusion.order within one fusion group");
        return failure();
      }
