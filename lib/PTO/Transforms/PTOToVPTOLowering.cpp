...
FailureOr<PTOLoopShape> resolveUnaryVecLoopShape(Operation *op, Value src,
                                                  Value dst, int64_t validCols) {
  if (op->hasAttr(kPTOLoweringChoiceAttrName)) {
    FailureOr<PTOLoweringChoiceAttr> choice = getVPTOLoweringChoiceAttr(op);
    if (failed(choice))
      return failure();
    return choice->getLoopShape();
  }

  FailureOr<PTOLoopShape> loopShape = selectVPTOLoopShapeForFullWidthCols(
      {deriveStaticTileCols(src), deriveStaticTileCols(dst)}, validCols);
  if (failed(loopShape))
    op->emitOpError("requires a resolvable VPTO loop shape");
  return loopShape;
}

FailureOr<PTOLoopShape> resolveBinaryVecLoopShape(Operation *op, Value src0,
  ...
  FailureOr<PTOLoopShape> loopShape = selectVPTOLoopShapeForSameShapeLinearPath(
...
FailureOr<PTOLoopShape> resolveExpandScalarVecLoopShape(Operation *op, Value dst,
                                                         int64_t validCols) {
  if (op->hasAttr(kPTOLoweringChoiceAttrName)) {
    FailureOr<PTOLoweringChoiceAttr> choice = getVPTOLoweringChoiceAttr(op);
    if (failed(choice))
  ...
  FailureOr<PTOLoopShape> loopShape =
      selectVPTOLoopShapeForFullWidthCols({deriveStaticTileCols(dst)}, validCols);
  if (failed(loopShape))
    op->emitOpError("requires a resolvable VPTO loop shape");
  return loopShape;
}
...
LogicalResult buildUnaryVecScope(StringRef family,
  ...
  llvm_unreachable("unsupported VPTO loop shape");
}
...
LogicalResult buildBinaryVecScope(StringRef family,
  ...
  auto buildBinaryValue = [&](Value lhs, Value rhs,
    ...
    if (family == "add")
      return rewriter.create<pto::VaddOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "sub")
      return rewriter.create<pto::VsubOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "mul")
      return rewriter.create<pto::VmulOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "div")
      return rewriter.create<pto::VdivOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "max")
      return rewriter.create<pto::VmaxOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "min")
      return rewriter.create<pto::VminOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "and")
      return rewriter.create<pto::VandOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "or")
      return rewriter.create<pto::VorOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    if (family == "xor")
      return rewriter.create<pto::VxorOp>(loc, vecType, lhs, rhs, predicate)
          .getResult();
    return failure();
  };
...
LogicalResult buildExpandScalarVecScope(const VPTOUnaryContract &contract,
                                        PTOLoopShape loopShape, Value scalar,
                                        Value dst,
                                        PatternRewriter &rewriter,
                                        Location loc) {
  auto vecType = getVPTOVRegType(rewriter.getContext(), contract.elementType);
...
LogicalResult buildScalarUnaryVecScope(StringRef family,
                                       const VPTOUnaryContract &contract,
                                       VPTOLoweringStrategy strategy,
                                       PTOLoopShape loopShape, Value src,
...
LogicalResult lowerTADD(TAddOp op, PatternRewriter &rewriter,
  ...
  if (failed(checkGenericBinaryContract(
    ...
    return failure();
  FailureOr<PTOLoopShape> loopShape = resolveBinaryVecLoopShape(
...
LogicalResult lowerTSelS(TSelSOp op, PatternRewriter &rewriter) {
  VPTOUnaryContract contract = buildUnaryContract("sels", op.getSrc());
  deriveValidShapeValues(op.getDst(), contract.validRowsValue, contract.validColsValue);
  deriveValidShape(op.getDst(), contract.validRows, contract.validCols);
  ...
  if (contract.tileDomain != VPTOTileDomain::Vec ||
      ...
      deriveTileDomain(getMemorySpace(op.getTmp())) != VPTOTileDomain::Vec ||
      deriveTileDomain(getMemorySpace(op.getDst())) != VPTOTileDomain::Vec)
  ...
  Value selected = rewriter
                       .create<pto::VselOp>(op.getLoc(), vecType, srcVec.getResult(),
...
LogicalResult lowerTAddC(TAddCOp op, PatternRewriter &rewriter) {
  ...
  if (failed(checkGenericBinaryContract(
          ...
          "f16, f32, bf16, and 8/16/32-bit integer element types")))
    return failure();
  FailureOr<PTOLoopShape> firstLoopShape = resolveBinaryVecLoopShape(
  ...
  if (failed(checkGenericBinaryContract(
          ...
          "f16, f32, bf16, and 8/16/32-bit integer element types")))
    return failure();
  FailureOr<PTOLoopShape> secondLoopShape = resolveBinaryVecLoopShape(
