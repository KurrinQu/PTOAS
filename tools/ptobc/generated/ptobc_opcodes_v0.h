...
inline constexpr OpInfo kOpTable[] = {
  ...
  {0x107D, "pto.aic_initialize_pipe", 0, 0x00, 0x02, 0, 0, 0, 0x00},
  {0x107E, "pto.aiv_initialize_pipe", 0, 0x00, 0x02, 0, 0, 0, 0x00},
  {0x107F, "pto.tpush_to_aiv", 0, 0x00, 0x00, 1, 0, 0, 0x00},
  {0x1080, "pto.tpush_to_aic", 0, 0x00, 0x00, 1, 0, 0, 0x00},
  {0x1081, "pto.tpop_from_aic", 0, 0x01, 0x04, 0, 1, 0, 0x08},
  {0x1082, "pto.tpop_from_aiv", 0, 0x01, 0x04, 0, 1, 0, 0x08},
  {0x1083, "pto.tfree_from_aic", 0, 0x00, 0x00, 0, 0, 0, 0x00},
  {0x1084, "pto.tfree_from_aiv", 0, 0x00, 0x00, 0, 0, 0, 0x00},
...
inline std::optional<uint16_t> lookupOpcodeByName(llvm::StringRef name) {
  uint16_t v = llvm::StringSwitch<uint16_t>(name)
    ...
    .Case("pto.aic_initialize_pipe", 0x107D)
    .Case("pto.aiv_initialize_pipe", 0x107E)
    .Case("pto.tpush_to_aiv", 0x107F)
    .Case("pto.tpush_to_aic", 0x1080)
    .Case("pto.tpop_from_aic", 0x1081)
    .Case("pto.tpop_from_aiv", 0x1082)
    .Case("pto.tfree_from_aic", 0x1083)
    .Case("pto.tfree_from_aiv", 0x1084)
    .Case("scf.for", 0x4000)
    .Case("scf.if", 0x4001)
...
inline std::optional<OpcodeAndVariant> lookupOpcodeAndVariantByFullName(llvm::StringRef fullName) {
  ...
  return llvm::StringSwitch<std::optional<OpcodeAndVariant>>(fullName)
    ...
    .Case("pto.aic_initialize_pipe", OpcodeAndVariant{0x107D, 0, 0})
    .Case("pto.aiv_initialize_pipe", OpcodeAndVariant{0x107E, 0, 0})
    .Case("pto.tpush_to_aiv", OpcodeAndVariant{0x107F, 0, 0})
    .Case("pto.tpush_to_aic", OpcodeAndVariant{0x1080, 0, 0})
    .Case("pto.tpop_from_aic", OpcodeAndVariant{0x1081, 0, 0})
    .Case("pto.tpop_from_aiv", OpcodeAndVariant{0x1082, 0, 0})
    .Case("pto.tfree_from_aic", OpcodeAndVariant{0x1083, 0, 0})
    .Case("pto.tfree_from_aiv", OpcodeAndVariant{0x1084, 0, 0})
